...
describe('Audit Logs (e2e)', () => {
  ...
  beforeEach(async () => {
    ...
    educatorToken = (
      await request(app.getHttpServer())
        .post('/api/auth/login')
        .send({ email: educator.email, password: TEST_PASSWORD })
    ).body.token;
  ...
  async function auditFor(module: string, entityId: string) {
    const res = await request(app.getHttpServer())
      .get(`/api/audit-logs?module=${module}`)
      .set('Authorization', auth(directorToken))
      .expect(200);
    return res.body.filter((l: { entityId: string }) => l.entityId === entityId);
  }
  ...
  describe('Finance', () => {
    ...
    it('logs a Transaction update with a before/after snapshot', async () => {
      const created = await request(app.getHttpServer())
        ...
        .set('Authorization', auth(directorToken))
        .send({
          type: 'DEPENSE', category: 'ENTRETIEN', label: 'Avant',
          amountXof: 1000, date: '2026-01-15',
        });
  ...
  describe('Maintenance', () => {
    it('logs a MaintenanceTicket create', async () => {
      const res = await request(app.getHttpServer())
        ...
        .set('Authorization', auth(directorToken))
        .send({ title: 'Fuite au robinet', spaceId, urgency: 'MOYENNE', reportedBy: 'Test' })
        .expect(201);
  ...
  describe('Administrative Procedures', () => {
    it('logs an AdministrativeProcedure create', async () => {
      const res = await request(app.getHttpServer())
        ...
        .set('Authorization', auth(directorToken))
        .send({ title: 'Agrément 2026', procedureType: 'AGREMENT', authority: 'Ministère' })
        .expect(201);
  ...
  describe('Supplier Contracts', () => {
    it('logs a SupplierContract create', async () => {
      const res = await request(app.getHttpServer())
        ...
        .set('Authorization', auth(directorToken))
        .send({
          supplierName: 'Fournisseur X', contractName: 'Contrat eau',
          category: 'EAU', startDate: '2026-01-01',
        })
        .expect(201);
  ...
  describe('Incidents', () => {
    it('logs an Incident create and a subsequent status change', async () => {
      const created = await request(app.getHttpServer())
        ...
        .set('Authorization', auth(directorToken))
        .send({
          title: 'Chute', type: 'COMPORTEMENT', description: 'd',
          signaledBy: 'x', priority: 'N3',
        })
        .expect(201);
      ...
      expect(entries.map((e: { action: string }) => e.action).sort()).toEqual(['CREATE', 'STATUS_CHANGE']);
      const statusChange = entries.find((e: { action: string }) => e.action === 'STATUS_CHANGE');
  ...
  describe('search and filters', () => {
    it('filters by module', async () => {
      ...
      const res = await request(app.getHttpServer())
        ...
        .set('Authorization', auth(directorToken))
        .expect(200);
      expect(res.body.every((l: { module: string }) => l.module === 'CONTACTS')).toBe(true);
    ...
    it('filters by user', async () => {
      await request(app.getHttpServer())
        ...
        .send({ title: 'x', type: 'AUTRE', description: 'd', signaledBy: 'x', priority: 'N3' });

      const res = await request(app.getHttpServer())
        .get(`/api/audit-logs?userId=${directorId}`)
        .set('Authorization', auth(directorToken))
        .expect(200);
      expect(res.body.every((l: { user: { id: string } | null }) => l.user?.id === directorId)).toBe(true);
    });
    ...
    it('applies a text search across entity/action', async () => {
      ...
      const res = await request(app.getHttpServer())
        ...
        .expect(200);
      expect(res.body.length).toBeGreaterThan(0);
      expect(res.body.every((l: { entity: string }) => l.entity.toLowerCase().includes('stockitem'))).toBe(true);
    });
  ...
  describe('users filter helper', () => {
    it('lists every system user', async () => {
      ...
      expect(names).toEqual(expect.arrayContaining(['Test Director', 'Test Supervisor', 'Test Educator']));
    });
  });
