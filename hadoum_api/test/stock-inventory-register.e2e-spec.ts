...
interface InventoryAssetResponse {
  id: string;
  spaceId: string | null;
  ...
  describe('Inventory', () => {
    it('creates an asset and transfers it (low value -> direct)', async () => {
      const createRes = (await request(app.getHttpServer())
        ...
        .expect(201)) as { body: InventoryAssetResponse };
      ...
      const transferRes = (await request(app.getHttpServer())
        ...
        .expect(200)) as { body: InventoryAssetResponse };
  ...
  describe('Register (entry logs)', () => {
    it('creates an immediate visitor entry, checks them out, and masks the identity document number in list views', async () => {
      const createRes = (await request(app.getHttpServer())
        ...
        .expect(201)) as { body: EntryLogResponse };
      ...
      const listRes = (await request(app.getHttpServer())
        .get('/api/entry-logs')
        .set('Authorization', `Bearer ${directorToken}`)
        .expect(200)) as { body: EntryLogResponse[] };
      const listed = listRes.body.find((e) => e.id === entryId);
      if (!listed) throw new Error('Created entry log not found in list');
