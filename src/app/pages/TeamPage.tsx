...
import {
  teamMembers as initialTeamMembers, TeamMember,
  candidatesData, Candidat,
  formerMembersData, FormerMember,
} from '../data/mockData';
import {
  Plus, Search, X, Mail, Edit3, Users, Check,
  UserMinus, UserCheck, Upload, ChevronRight, ArrowUpRight,
  Paperclip,
} from 'lucide-react';
      ...
      <div className="w-full max-w-sm rounded-2xl overflow-hidden shadow-xl" style={{ background: '#FFFFFF' }}>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <button type="button"
              ...
              <Upload size={14} style={{ color: form.cvUploaded ? '#065F46' : '#9CA3AF', flexShrink: 0 }} />
              <span style={{ flex: 1, color: form.cvUploaded ? '#065F46' : '#6B7280', fontSize: 13 }}>
                {form.cvUploaded ? 'CV_joint.pdf — joint ✓' : 'Cliquer pour joindre un CV (simulé)'}
              </span>
              {form.cvUploaded && (
                <button type="button" onClick={e => { e.stopPropagation(); set('cvUploaded', false); }}
        ...
        <div className="flex gap-2 px-6 py-4" style={{ borderTop: '1px solid #F3F4F6', background: '#F9F7F3' }}>
          ...
          <button disabled={!canSave}
            onClick={() => canSave && onSave({ ...form, dateCandidate: new Date().toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric' }) })}
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <button type="button"
              ...
              style={{ border: `1px solid ${cvUploaded ? '#A7F3D0' : '#E5E7EB'}`, background: cvUploaded ? 'rgba(6,95,70,0.04)' : '#FAFAFA', cursor: 'pointer' }}>
              ...
              <span style={{ flex: 1, color: cvUploaded ? '#065F46' : '#6B7280', fontSize: 13 }}>
                {cvUploaded ? 'CV_joint.pdf — joint ✓' : 'Joindre un CV (simulé)'}
              </span>
              {cvUploaded && (
                <button type="button" onClick={e => { e.stopPropagation(); setCvUploaded(false); }}
                  style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
                  <X size={13} style={{ color: '#9CA3AF' }} />
                </button>
              )}
...
function CandidatesTab({ candidates, onAdd, onPromote, onEdit }: {
  ...
  const handleCvClick = (c: Candidat) => {
    if (c.cvUploaded) {
      toast('Ouverture du CV en cours…', { duration: 2000 });
    }
  };
...
export function TeamPage() {
  ...
  const [candidates, setCandidates] = useState<Candidat[]>(candidatesData);
  const [former, setFormer]         = useState<FormerMember[]>(formerMembersData);

  // Modals
  const [exitTarget, setExitTarget]             = useState<TeamMember | null>(null);
  const [editTarget, setEditTarget]             = useState<{ mode: 'edit'; member: TeamMember } | { mode: 'add' } | null>(null);
  ...
  const handleExit = (motif: string) => {
    ...
    const parts = exitTarget.name.split(' ');
    const initials = `${(parts[0]?.[0] ?? 'X').toUpperCase()}${(parts[1]?.[0] ?? 'X').toUpperCase()}`;
    setFormer(prev => [{
      id: Date.now(),
      name: exitTarget.name,
      role: exitTarget.role,
      dateSortie: new Date().toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric' }),
      motifSortie: motif,
      initials,
    }, ...prev]);
    setMembers(prev => prev.filter(m => m.id !== exitTarget.id));
    setExitTarget(null);
  };

  const handleEditSave = (form: { nom: string; prenom: string; poste: string; statut: TeamMember['status'] }) => {
    const fullName = `${form.prenom} ${form.nom}`.trim() || 'Nouveau membre';
    const initials = `${(form.prenom[0] ?? 'N').toUpperCase()}${(form.nom[0] ?? 'M').toUpperCase()}`;
    if (editTarget?.mode === 'edit') {
      setMembers(prev => prev.map(m =>
        m.id === editTarget.member.id ? { ...m, name: fullName, role: form.poste || m.role, status: form.statut, initials } : m
      ));
    } else {
      const newMember: TeamMember = {
        id: Math.max(...members.map(m => m.id), 0) + 1,
        name: fullName, role: form.poste || 'Éducateur',
        classes: [], status: form.statut,
        phone: '—', email: '—',
        since: new Date().toISOString().split('T')[0], initials,
      };
  ...
  const handlePromote = (id: number) => {
    ...
    if (!c) return;
    const initials = `${(c.prenom[0] ?? 'N').toUpperCase()}${(c.nom[0] ?? 'M').toUpperCase()}`;
    const newMember: TeamMember = {
      id: Math.max(...members.map(m => m.id), 0) + 1,
      name: `${c.prenom} ${c.nom}`,
      role: c.posteVise || 'Éducateur',
      classes: [], status: 'present',
      phone: c.telephone, email: '—',
      since: new Date().toISOString().split('T')[0], initials,
    };
    setMembers(prev => [newMember, ...prev]);
  ...
  // Réintégration (point 3)
  const handleReintegrate = (poste: string, date: string, _note: string) => {
    if (!reintegrateTarget) return;
    const parts = reintegrateTarget.name.split(' ');
    const initials = `${(parts[0]?.[0] ?? 'X').toUpperCase()}${(parts[1]?.[0] ?? 'X').toUpperCase()}`;
    const newMember: TeamMember = {
      id: Math.max(...members.map(m => m.id), 0) + 1,
      name: reintegrateTarget.name,
      role: poste || reintegrateTarget.role,
      classes: [], status: 'present',
      phone: '—', email: '—',
      since: date, initials,
    };
    setMembers(prev => [newMember, ...prev]);
    setFormer(prev => prev.filter(m => m.id !== reintegrateTarget.id));
  ...
  return (
    <div className="px-4 md:px-6 py-6 space-y-5" style={{ maxWidth: 1200 }}>
      ...
      {showAddCandidate && (
        <AddCandidateModal
          onSave={c => { setCandidates(prev => [{ ...c, id: Date.now() }, ...prev]); setShowAddCandidate(false); }}
      ...
      {editCandidate && (
        <EditCandidateModal
          ...
          onSave={updated => { setCandidates(prev => prev.map(c => c.id === updated.id ? updated : c)); setEditCandidate(null); }}
